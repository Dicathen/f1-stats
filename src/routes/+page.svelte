			...
			{:else if currentStandings.length > 0}
				<div class="space-y-2">
					{#each currentStandings as standing}
						<a href="/drivers/{standing.Driver.driverId}" class="flex items-center justify-between p-4 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
							<div class="flex items-center gap-4">
								...
								<div>
									...
									<p class="font-semibold" style="color: {getTeamColor(standing.Constructors[0]?.name)}">{standing.Driver.givenName} {standing.Driver.familyName}</p>
			...
			{:else if recentRaces.length > 0}
				<div class="grid gap-4 md:grid-cols-3">
					{#each recentRaces as race}
						<a href="/races/{getRaceSlug(race)}" class="p-4 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
							<h3 class="font-semibold mb-1">{race.raceName}</h3>
							<p class="text-sm text-muted-foreground mb-2">{race.Circuit.circuitName}</p>
							<p class="text-xs text-muted-foreground mb-3">{new Date(race.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
							{#if race.Results && race.Results[0]}
								<div class="flex items-center gap-2">
									<div class="w-2 h-2 rounded-full bg-primary"></div>
									<!-- Apply team color to winner name -->
									<p class="text-sm font-medium" style="color: {getTeamColor(race.Results[0].Constructor?.name)}">{race.Results[0].Driver.givenName} {race.Results[0].Driver.familyName}</p>
								</div>
							{/if}
						</a>
					{/each}
